...
def myprosody(m, p):
    ...
    # Updated file paths
    outo = os.path.join(p, "..", "datanewchi22.csv")
    outst = os.path.join(p, "..", "datanewchi44.csv")
    outsy = os.path.join(p, "..", "datanewchi33.csv")
    pa2 = os.path.join(p, "..", "stats.csv")
    ...
    try:
        ...
        df1 = pd.read_csv(outsy, names=['avepauseduratin', 'avelongpause', 'speakingtot', 'articulationrate', 'mr', 'q50', 'std', 'fmax', 'fmin', 'vowelinx2', 'formantmean', 'formantstd', 'ins'])
        df1 = df1.dropna()
        prosody_results = df1.to_dict(orient='records')[0]
        ...
        for i, name in enumerate(nsns):
            sl0 = dataframe.iloc[4:7, i + 1]
            score = prosody_results.get(name, None)
            he = scipy.stats.percentileofscore(sl0, score, kind='strict') if score is not None else None
            prosody_results[name] = {"score": score, "percentile": he}
